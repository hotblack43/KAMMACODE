...
PRO gofindpointers,nrows,idx,jdx
 ...
 ; X
 x_orig=reform(array(2:nvars,*)) ; SKIPPING CARBS!!!!!
...
;openw,44,'x.dat'
;printf,44,format='(8(f15.7,1x))',x_orig
;close,44
;openw,44,'y.dat'
;printf,44,y_orig
;close,44
     coef=regress(x_orig(*,*),y_orig,/double,const=intercept,yfit=yhat,sigma=coef_si)
     if (itrial eq 0) then begin
       delta=y_orig-yhat
       sd=stddev(delta)
       idx_poor=where(abs(delta) gt sd*2)
       idx_good=where(abs(delta) le sd*2)
       plot,xrange=[0,20],yrange=[0,20],xstyle=3,ystyle=3,y_orig(idx_good),yhat(idx_good),psym=7,xtitle='Observed IG',title='Multivariate Regression Model',ytitle='Model IG'
       ...
       oplot,[0,20],[sd*2,20+sd*2],linestyle=1
;help, x_orig(*,idx),y_orig(idx),yhat,delta,idx_poor,idx_good
;stop

       plot,yrange=[0,20],xstyle=3,ystyle=3,array(6,idx_good),yhat(idx_good),psym=7,xtitle='FIXME',ytitle='Model IG'
...
; !P.color=pcolor
 ...
 for j=1,n_elements(varnames)-2,1 do print,format='(a,a,a,f6.2)','Z for ',varnames(j+1),' : ',1./avg(z_list(j+1),1)
 return
 end
