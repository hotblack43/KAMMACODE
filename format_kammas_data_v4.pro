...
;        if (idx(0) ne -1 or jdx(0) ne -1) then begin
	...
	CC=total(carbs(1,idx)*hat(60.*24.*dt_carbs))
endif
	...
	BB=total(bolus(1,jdx)*hat(60.*24.*dt_bolus))
             ...
             printf,44,tnow-long(tnow),CC,BB,ig(1,i)
             ;printf,44,tnow-long(tnow),total(carbs(1,idx)*hat(60.*24.*dt_carbs)),total(bolus(1,jdx)*hat(60.*24.*dt_bolus)),ig(1,i)
;            endif
         endif
     endfor
 close,44
...
;print,res(2),' +/- ',sigs(2)
 ...
 for i=0,n-1,1 do begin
     ...
     if (c(i) ne 0 and b(i) ne 0) then printf,55,t(i),c(i),b(i),ig(i)-ig_previous
     endfor
 close,55
; plot
